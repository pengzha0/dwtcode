...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.DWTCODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.DWTCODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.DWTCODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".dwtcode") && dir !== Flag.DWTCODE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old dwtcode.json shape) still get their settings applied.
    const normalized = (() => {
