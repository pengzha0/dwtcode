...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from dwtcode.json files
...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.DWTCODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("dwtcode", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "dwtcode")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "dwtcode"))
  }
  if (Flag.DWTCODE_CONFIG) files.push(Flag.DWTCODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "dwtcode"))
