...
export const RunCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running dwtcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to DWTCODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.DWTCODE_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.DWTCODE_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.DWTCODE_SERVER_USERNAME ?? "dwtcode"
