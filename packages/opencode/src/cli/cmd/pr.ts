  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found dwtcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["dwtcode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting dwtcode...")
        UI.println()

        // Launch dwtcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const dwtcodeArgs = sessionId ? ["-s", sessionId] : []
        const dwtcodeProcess = spawn("dwtcode", dwtcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          dwtcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`dwtcode exited with code ${code}`))
          })
          dwtcodeProcess.on("error", reject)
        })
      },
    })
  },
